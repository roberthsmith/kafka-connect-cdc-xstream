  ...
  public SuccessOrFailure isClusterHealthy(Cluster cluster) {
    return SuccessOrFailure.onResultOf(new Attempt() {
      ...
      public boolean attempt() throws Exception {
        Container oracleContainer = cluster.container(Constants.ORACLE_CONTAINER);
        DockerPort dockerPort = oracleContainer.port(Constants.ORACLE_PORT);
        if (!dockerPort.isListeningNow()) {
          if (log.isTraceEnabled()) {
            log.trace("Port {} is not listening on container {}.", Constants.ORACLE_PORT, Constants.ORACLE_CONTAINER);
          }
          return false;
        }
